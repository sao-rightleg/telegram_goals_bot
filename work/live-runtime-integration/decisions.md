...
**Retry 2026-07-19 provider fixed:** After the user fixed `TRANSCRIPTION_PROVIDER`, reran `Deploy Test` as run `29697791616`. The workflow passed upload, remote install, VPS tests (`351 passed in 15.72s`), and runtime storage readiness. `check-config` now fails on Google Sheets readiness with sanitized `Google Sheets request failed: HttpError`, so the remaining blocker is Google Sheets ID/access/API/schema configuration for the test spreadsheet.
