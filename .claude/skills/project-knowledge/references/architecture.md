...
- Captain flows: captain service resolves Telegram identity, consent, `role = captain`, and captain `team_id`; own-team views list only rows from the captain's team and avoid internal ID leakage. Captains can inspect current progress and select an authorized team participant to view the active goal or its numbered planned steps in private chat. Captain manual reports reuse weekly report draft state with `flow_source = captain_manual`, selected participant state, and captain submitter metadata. Final manual reports write normal `weekly_reports` and `weekly_report_steps` business facts for the selected participant, revalidate own-team ownership, dropped status, duplicate report, deadline, active goal, selected status, selected steps, and non-empty text before save, and clear draft state after success.
